...
class ResNet(SegmentationBaseModel):
    ...
    def model(self, x_in, keep_prob=0.5, reuse=False, training=True):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            signal = self._residual_block(signal, self.kernels[-1], block=self.modules-1, stacks=self.stacks, name_scope='r')
            # signal = tf.contrib.nn.alpha_dropout(signal, keep_prob=keep_prob)
            # signal = conv(signal, self.kernels[-1], upsample_rate=2, k_size=1, var_scope='intermediate')
            ...
            block_name = 'd0_residual'
            signal = self._residual_block(signal, self.kernels[0],
                block=0, stacks=self.stacks, name_scope='d')
